...
public class AlphaBeta {
    ...
    //Logic behind placing the actual move
    ...
    private static int alphaBeta(Board board, Color player, int depth, int alpha, int beta, Vector<Move> resultMoveSeq){
        ...
        if(player == Color.white)
        {
            for(int i=0; i<boardD.size(); i++)
            {
                ...
                //determinating the best move
                if(value > alpha)
                {
                    alpha = value;
                    bestMoveSeq = moveSeq;
                }
                //stop checking the branch
                if(alpha>beta)
                {
                    break;
                }
    ...
    //Supportive function for the algorithm
    ...
    //Supporting funcion for the algorithm that evaluates values
    ...
    //Assigning values to the pieces in order to support decision making
